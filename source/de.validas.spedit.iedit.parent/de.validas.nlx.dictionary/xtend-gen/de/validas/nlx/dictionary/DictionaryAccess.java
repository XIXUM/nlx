...
public class DictionaryAccess implements IDictionaryAccess {
  ...
  public void dictPostProcess(final String wordToLc, final String type) {
    ...
    if (Objects.equal(type, DictionaryConstants.NOUN_)) {
      _matched=true;
    ...
    if (!_matched) {
      if (Objects.equal(type, DictionaryConstants.VERB_)) {
        _matched=true;
    ...
    if (!_matched) {
      if (Objects.equal(type, DictionaryConstants.ADJECTIVE_)) {
        _matched=true;
  ...
  public List<String> listAllLabel(final String label) {
    ...
    if (_not) {
      final Function<Record, String> _function = (Record rec) -> {
        return java.util.Objects.toString(rec.values().get(0).asString(), null);
      };
  ...
  public void addSuccessor(final DictItem source, final DictItem target, final Set<String> targetTypes, final List<ITypeAttributes> attribs) throws ClientException {
    ...
    boolean _matched = false;
    if (Objects.equal(_type, NodeConstants._INTERPUNCTION)) {
      _matched=true;
      StringConcatenation _builder = new StringConcatenation();
      {
            ...
            for (final IRelationshipEL rel : rels) {
              {
                ...
                if (_not) {
                  ...
                  if (_contains) {
                    ...
                    boolean _equals_2 = Objects.equal(aT, dir);
                    if (_equals_2) {
          ...
          for (final IRelationshipEL r : _get) {
            {
              ...
              if (_greaterThan_1) {
                for (final Map el : filRes) {
                  {
                    ...
                    boolean _matched = false;
                    if (Objects.equal(elS, DictionaryConstants._END)) {
                      _matched=true;
                      endMap.get(DictionaryConstants._INCLUDED).add(r);
                    }
                    if (!_matched) {
                      if (Objects.equal(elS, DictionaryConstants._START)) {
                        _matched=true;
                ...
                boolean _matched = false;
                if (Objects.equal(d, DictionaryConstants._END)) {
                  _matched=true;
                  endMap.get(DictionaryConstants._LEFT_OVER).add(r);
                }
                if (!_matched) {
                  if (Objects.equal(d, DictionaryConstants._START)) {
                    _matched=true;
